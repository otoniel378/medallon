"""
05_Mix_de_Productos.py — Mix de productos, especificaciones tecnicas y cross-sell.
"""
...
from config import APP_NAME, COLORS, COLOR_SEQUENCE, HEATMAP_COLORSCALE
...
from analytics.mix_productos import (
...
st.set_page_config(
    page_title=f"Mix de Productos — {APP_NAME}",
    page_icon="🎯",
    layout="wide",
)
...
min_clientes_combo = st.sidebar.slider(
    "Min. clientes para combos",
    min_value=1, max_value=20, value=2, key="mix_min_clientes",
)
...
anios_mix = []
if not df_vl.empty and "FECHAEMB" in df_vl.columns:
    anios_mix = sorted(df_vl["FECHAEMB"].dropna().dt.year.unique().tolist(), reverse=True)
...
# ---------------------------------------------------------------------------
# KPIs (usando datos base sin filtro temporal)
# ---------------------------------------------------------------------------
...
if not df_mg.empty and "PRODUCTO_LIMPIO" in df_mg.columns:
    ...
    tabla_ejecutiva(
        df_part_res,
        titulo="Participacion por producto en el periodo seleccionado",
...
# ============================================================
# SECCION 2 — Analisis de Especificaciones Tecnicas
# ============================================================
seccion_titulo(
    "Analisis de Especificaciones Tecnicas",
...
if not df_vl.empty and "CALIBRE" in df_vl.columns and "PROCESO" in df_vl.columns:
    # Filtros inline de esta sección
    col_e1, col_e2, col_e3 = st.columns([1, 1, 2])
    with col_e1:
        anio_esp = st.selectbox(
            "Año",
            options=["Todos"] + [str(a) for a in anios_mix],
            key="esp_anio",
        )
    with col_e2:
        mes_esp_nombre = st.selectbox(
            "Mes",
            options=meses_nombres,
            key="esp_mes",
            disabled=(anio_esp == "Todos"),
        )

    # Construir df_vl_base con filtro temporal
    ...
    if df_vl_esp.empty:
        ...
        with col_e5:
            ...
            if cal_min < cal_max:
                cal_rango = st.slider(
                    ...
                    value=(cal_min, cal_max), step=0.1,
                    key="esp_calibre",
                )
            else:
        ...
        df_esp = df_esp[
            (df_esp["CALIBRE"] >= cal_rango[0]) &
            (df_esp["CALIBRE"] <= cal_rango[1])
        ]
        ...
        if df_esp.empty:
            ...
            # Grafico A: Box plot calibres por proceso
            col_g1, col_g2 = st.columns(2)
            with col_g1:
                seccion_titulo("Distribucion de Calibres por Proceso", "")
                fig_box = px.box(
                    df_esp, x="PROCESO", y="CALIBRE",
                    color="PROCESO",
                    color_discrete_sequence=COLOR_SEQUENCE,
                    labels={"CALIBRE": "Calibre (mm)", "PROCESO": ""},
                    points=False,
                )
            ...
            with col_g2:
                ...
                fig_cal = px.bar(
                    df_calp, x="RANGO_CAL", y="PESO_TON", color="PROCESO",
                    barmode="group",
            ...
            # Grafico C: Top combinaciones PROCESO + CALIBRE
            ...
            with col_c1:
                fig_tc = go.Figure(go.Bar(
                    x=df_combo_c["PESO_TON"].iloc[::-1],
                    y=df_combo_c["COMBO"].iloc[::-1],
                    orientation="h",
                    marker=dict(
                        color=df_combo_c["PESO_TON"].iloc[::-1],
                        colorscale=HEATMAP_COLORSCALE,
                        showscale=True,
                        colorbar=dict(title="ton", thickness=12),
                    ),
                    text=df_combo_c["PESO_TON"].iloc[::-1].round(0),
            ...
            # Grafico D: Combinacion especifica PROCESO + CALIBRE + ANCHO
            ...
            if not df_ancho.empty:
                ...
                df_c3["ESPECIFICACION"] = (
                    df_c3["PROCESO"] + "  |  " +
                    df_c3["CALIBRE"].apply(lambda x: f"{x:.2f}") + "mm × " +
...
# ============================================================
# SECCION 3 — Co-ocurrencia (usa datos base sin filtro temporal)
# ============================================================
...
if not df_cs.empty:
    tabla_ejecutiva(
        df_cs, col_formatos={"N_CLIENTES_LO_COMPRAN": "{:,}"},
